...
import { 
  ...
  Home, Calendar, Settings, User, BarChart3, ChevronRight 
} from 'lucide-react';
...
export default function App() {
  ...
  // 1. NAVIGATION STATE (NEW)
  const [activeTab, setActiveTab] = useState('dashboard'); // 'dashboard', 'stats', 'calendar', 'profile'
  ...
  const StatsCard = ({ icon: Icon, label, value, sub }) => (
  ...
  const CycleProgress = () => {
    ...
    const percentage = Math.round(((dayIndex + 1) / cycleDuration) * 100);
    ...
    return (
      <div className="bg-black dark:bg-black border border-zinc-800 text-white p-6 rounded-3xl shadow-2xl h-full flex flex-col justify-between relative overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1">Cycle Progress</h3>
          <div className="flex items-baseline gap-2 mb-4">
             <span className="text-5xl font-bold tracking-tighter">{percentage}%</span>
             <span className="text-accent text-sm font-bold">COMPLETE</span>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatsCard icon={Scale} label="Weight" value={`${Number(weightChange) > 0 ? '+' : ''}${weightChange}kg`} sub="Since Start" />
        </div>

        {/* Main Content Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Left Column (Main Workout) */}
  ...
  const ProfileView = () => (
    <div className="max-w-md mx-auto bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-gray-100 dark:border-zinc-800 text-center mt-10">
       <div className="w-24 h-24 mx-auto bg-gray-100 dark:bg-zinc-800 rounded-full flex items-center justify-center text-zinc-400 mb-6 text-3xl font-bold border-4 border-white dark:border-black shadow-xl">
         {user.name.charAt(0)}
       </div>
       ...
       <div className="mt-8 text-left space-y-4">
         <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-zinc-950 rounded-xl">
            <span className="text-sm font-bold text-zinc-500">Diet</span>
            <span className="font-bold capitalize">{user.dietPreference}</span>
         </div>
         <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-zinc-950 rounded-xl">
            <span className="text-sm font-bold text-zinc-500">Activity</span>
            <span className="font-bold capitalize">{user.activityLevel}</span>
         </div>
         <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-zinc-950 rounded-xl">
            <span className="text-sm font-bold text-zinc-500">Daily Target</span>
            <span className="font-bold text-accent">{user.calorieTarget} kcal</span>
         </div>
       </div>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-black text-black dark:text-white font-sans transition-colors duration-300 selection:bg-accent selection:text-white overflow-hidden">
      ...
      <aside className="hidden md:flex w-24 flex-col items-center py-8 bg-white dark:bg-zinc-950 border-r border-gray-200 dark:border-zinc-900 z-20">
        <div className="w-10 h-10 bg-black dark:bg-white rounded-xl flex items-center justify-center text-white dark:text-black mb-12 shadow-lg">
          <Parentheses size={20} />
        </div>
        
        <nav className="flex-1 flex flex-col gap-6 w-full px-4">
          <button onClick={() => setActiveTab('dashboard')} className={`p-3 rounded-xl transition-all flex justify-center ${activeTab === 'dashboard' ? 'bg-accent text-white shadow-lg shadow-accent/30' : 'text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
            <Home size={22} />
          </button>
          <button onClick={() => setActiveTab('stats')} className={`p-3 rounded-xl transition-all flex justify-center ${activeTab === 'stats' ? 'bg-accent text-white shadow-lg shadow-accent/30' : 'text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
            <BarChart3 size={22} />
          </button>
          <button onClick={() => setActiveTab('calendar')} className={`p-3 rounded-xl transition-all flex justify-center ${activeTab === 'calendar' ? 'bg-accent text-white shadow-lg shadow-accent/30' : 'text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
            <Calendar size={22} />
          </button>
          <button onClick={() => setActiveTab('profile')} className={`p-3 rounded-xl transition-all flex justify-center ${activeTab === 'profile' ? 'bg-accent text-white shadow-lg shadow-accent/30' : 'text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
            <User size={22} />
          </button>
        </nav>

        <div className="flex flex-col gap-4 w-full px-4">
          <button onClick={toggleTheme} className="p-3 rounded-xl text-zinc-400 hover:text-accent transition-colors flex justify-center">
             {theme === 'dark' ? <Sun size={22} /> : <Moon size={22} />}
          </button>
          <button onClick={handleLogout} className="p-3 rounded-xl text-zinc-400 hover:text-red-500 transition-colors flex justify-center">
             <LogOut size={22} />
          </button>
        </div>
      </aside>
            ...
            <div className="flex justify-between items-end mb-8 md:mb-10">
              <div>
                <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2">
                  {activeTab === 'dashboard' && `Good Morning, ${user.name.split(' ')[0]}.`}
                  {activeTab === 'stats' && 'Your Statistics.'}
                  {activeTab === 'calendar' && 'History.'}
                  {activeTab === 'profile' && 'Athlete Profile.'}
                </h1>
                {activeTab === 'dashboard' && (
